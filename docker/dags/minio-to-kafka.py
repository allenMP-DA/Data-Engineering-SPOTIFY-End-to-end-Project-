...
def extract_from_minio():
    ...
    print(
        f"✅ Extracted {len(all_events)} events from MinIO and saved to {LOCAL_TEMP_PATH}"
    )
...
def load_raw_to_snowflake(**context):
    ...
    print(
        f"✅ Loaded {len(events)} raw records into Snowflake table: {SNOWFLAKE_TABLE}"
    )
...
with DAG(
    ...
    extract_task = PythonOperator(
        task_id="extract_data", python_callable=extract_from_minio
    )
